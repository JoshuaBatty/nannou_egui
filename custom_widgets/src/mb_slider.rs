...
impl<'a> MbSlider<'a> {
    ...
    pub fn new<Num: egui::emath::Numeric>(value: &'a mut Num, range: RangeInclusive<Num>) -> Self {
    ...
    fn set_value(&mut self, mut value: f64) {
        ...
        if let Some(max_decimals) = self.max_decimals {
            value = egui::emath::round_to_decimals(value, max_decimals);
        }
    ...
    fn value_from_position(&self, position: f32, position_range: RangeInclusive<f32>) -> f64 {
        let normalized = egui::remap_clamp(position, position_range, 0.0..=1.0) as f64;
    ...
    fn position_from_value(&self, value: f64, position_range: RangeInclusive<f32>) -> f32 {
        ...
        egui::lerp(position_range, normalized as f32)
    }
}
...
impl<'a> MbSlider<'a> {
    ...
    /// Just the slider, no text
    fn slider_ui(&mut self, ui: &mut egui::Ui, response: &egui::Response) {
        ...
        response.widget_info(|| egui::WidgetInfo::slider(value, &self.text));

        if response.has_focus() {
            let (dec_key, inc_key) = (egui::Key::ArrowLeft, egui::Key::ArrowRight);
    ...
    fn marker_center(&self, position_1d: f32, rail_rect: &egui::Rect) -> egui::Pos2 {
        egui::pos2(position_1d, rail_rect.center().y)
    }

    fn pointer_position(&self, pointer_position_2d: egui::Pos2) -> f32 {
...
impl<'a> egui::Widget for MbSlider<'a> {
    fn ui(mut self, ui: &mut egui::Ui) -> egui::Response {
        let old_value = self.get_value();

        let mut response = self.add_contents(ui);
